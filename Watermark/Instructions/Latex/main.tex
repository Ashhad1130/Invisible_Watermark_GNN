...
%-- Full-width title block (one-column preamble) --%
\twocolumn[{%
\begin{center}
  {\LARGE\bfseries
    Invisible Watermarking for AI-Generated Images:\\[5pt]
    Replicating and Extending Tree-Ring Watermarks\\[5pt]
    with Dual-Band and All-Channel Fourier Embedding\par}
  \vspace{10pt}
  {\normalsize
    \textbf{Ashhad Raza Quadri}~(Matriculation:~4756039,~\textit{Primary Author}) \quad
    Maria Alejandra Pabon Galindo~(Matriculation:~4756044) \quad
    Saliq~(Matriculation:~11111)\par}
  \vspace{4pt}
  {\small
    \textit{Generative Neural Networks for the Sciences,
    Winter Semester 2025/26,
    Ruprecht-Karls-Universit\"at Heidelberg.}\\
    Repository:~\url{https://github.com/Ashhad1130/Invisible_Watermark_GNN} \quad Submitted: March 30, 2026.\par}
  \vspace{8pt}
\end{center}
...
\subsection{Societal Context and Regulatory Landscape}

The capacity of modern generative models to synthesise photorealistic
images has grown dramatically over the past five years.
Latent diffusion models (LDMs) such as Stable
Diffusion~\cite{rombach2022ldm} and Imagen~\cite{saharia2022imagen}
can now produce images that are indistinguishable from photographs in
blind human evaluations, with creative quality rivalling or surpassing
...
publications; non-consensual intimate synthetic imagery has been weaponised
for harassment; and misinformation campaigns now use AI-generated
photographs as apparent visual evidence.
...
Geometric transformations (rotation, crop, scaling) misalign the
...
embedding resolve all Baseline FAIL conditions on crop and rotation
...
\textbf{Maria Alejandra} conducted the literature survey and wrote
Related Work, Background, and Conclusions.
...
Invisible watermarking is inherently a surveillance technology: it
allows the service provider to track provenance of every generated
image without the creator's awareness.
...
Fairness is a secondary concern: if non-watermarked image Fourier spectra
...
\begin{enumerate}
  \item \textbf{Source-side Fourier watermarking works robustly at scale.}
    ...
    achieve AUC $= 1.0$, confirming that the method is production-ready
    for these common attack modes.
